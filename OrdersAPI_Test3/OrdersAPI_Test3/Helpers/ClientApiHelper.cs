...
namespace OrdersAPI_Test3.Helpers
{
    public class ClientApiHelper
    {
        ...
        public void GetToken(string _username, string _password, string partialUri = "token")
        {
            ...
            using (HttpClient httpClient = new HttpClient())
            {
                ...
                using (HttpRequestMessage request =
                    new HttpRequestMessage(HttpMethod.Post, new Uri(baseAddress, partialUri)))
                {
                    request.Content =
        ...
        public dynamic PostAsync(string partialUri, Dictionary<string, string> paramenters, bool needToken=false)
        {
            ...
            using (HttpClient httpClient = new HttpClient())
            {
                ...
                {
                    request.Content = new FormUrlEncodedContent(paramenters);
                    var response = httpClient.PostAsync(request.RequestUri, request.Content).Result;
